...
#银行行业当日总市值 7197166841786.770
if __name__ == '__main__':
    db = DB()  # 证券代码
    # sql_zsz = "SELECT SUM(TOTMKTCAP)  FROM zgzb_zx.TQ_SK_FININDIC WHERE   TRADEDATE='20211215'   and  SYMBOL IN (SELECT SYMBOL  FROM zgzb_zx.TQ_SK_BASICINFO WHERE SWLEVEL1CODE = '480000' AND SWLEVEL1NAME IS NOT NULL AND SWLEVEL1CODE IS NOT NULL AND SWLEVEL1NAME != '' AND SWLEVEL1CODE != '' AND   SWLEVEL1CODE = '480000' AND SETYPE = 101 AND ISVALID = 1 AND LISTSTATUS = 1);"
    # data_zsz = db.find_all(sql_zsz)
    sql_xdg = "SELECT SUM(TOTMKTCAP)  FROM zgzb_zx.TQ_SK_FININDIC WHERE   TRADEDATE='20211215'   and  SYMBOL IN (SELECT DISTINCT (SAMPLECODE) FROM zgzb_zx.TQ_IX_COMP tic WHERE SYMBOL =000300 AND USESTATUS = 1)"
    data_xdg = db.find_all(sql_xdg)
    print("沪深300当日总市值", data_xdg[0]["SUM(TOTMKTCAP)"])

    # print("相对估值",data_zsz[0]["SUM(TOTMKTCAP)"]/data_xdg[0]["SUM(TOTMKTCAP)"])


    sql2 = "SELECT COMPCODE FROM zgzb_zx.TQ_SK_BASICINFO WHERE SWLEVEL1CODE = '480000' AND SWLEVEL1NAME IS NOT NULL AND SWLEVEL1CODE IS NOT NULL AND SWLEVEL1NAME != '' AND SWLEVEL1CODE != '' AND SWLEVEL1CODE = '480000' AND SETYPE = 101 AND ISVALID = 1 AND LISTSTATUS = 1;  "
